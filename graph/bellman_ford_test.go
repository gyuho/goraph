...
func TestBellmanFord12(t *testing.T) {
	...
	pathSlice, mapToDistance, noNegCycle := data.BellmanFord(data.GetNodeByID("S"), data.GetNodeByID("T"))
	ts := []string{}
	...
	if mapToDistance[data.GetNodeByID("T")] != -2.0 {
		t.Errorf("Expected the shortest distance of -2.0 (from S to T) but %.2f", mapToDistance[data.GetNodeByID("T")])
	}
	if !noNegCycle {
