...
type Set map[*gs.Vertex]*gs.Vertex
...
func (s Set) Insert(vertices ...*gs.Vertex) {
...
func InstantiateSet(vertices ...*gs.Vertex) Set {
	n := NewSet()
	for _, value := range vertices {
		n.Insert(value)
	}
...
func (s Set) GetElements() []*gs.Vertex {
	slice := []*gs.Vertex{}
	for key := range s {
...
func (s Set) Contains(vtx *gs.Vertex) bool {
...
func (s Set) Delete(vtx *gs.Vertex) bool {
	if _, exist := s[vtx]; exist {
		delete(s, vtx)
		return true
	} else {
...
func (s Set) Intersection(a Set) []*gs.Vertex {
	slice := []*gs.Vertex{}
	for key := range a {
...
func (s Set) Union(a Set) []*gs.Vertex {
	slice := []*gs.Vertex{}
	for key := range s {
		slice = append(slice, key)
	}
...
func (s Set) Subtract(a Set) []*gs.Vertex {
	slice := []*gs.Vertex{}
	for key := range s {
