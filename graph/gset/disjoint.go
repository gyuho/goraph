...
// DisJointSet is two sets that have no element in common.
// That is, their intersection is the empty set.
type DisJointSet struct {
	Rep  *gs.Vertex // Representing Vertex
...
func makeSet(vtx *gs.Vertex) Set {
	set := make(Set)
	set[vtx] = vtx
	return set
}
...
func MakeSet(vtx *gs.Vertex) *DisJointSet {
	return &DisJointSet{
		Rep:  vtx,
		Mems: makeSet(vtx),
	}
...
func MakeGraphSet(graph *gs.Graph) []*DisJointSet {
	vertices := graph.GetVertices()
	result := []*DisJointSet{}
	for _, vtx := range *vertices {
		result = append(result, MakeSet(vtx.(*gs.Vertex)))
	}
	return result
}

// SetContains returns true if the value exists in the set.
func (s DisJointSet) SetContains(vtx *gs.Vertex) bool {
...
func GetSet(vtx *gs.Vertex, ds []*DisJointSet) *DisJointSet {
	for _, set := range ds {
...
func FindSet(vtx *gs.Vertex, ds []*DisJointSet) *gs.Vertex {
	set := GetSet(vtx, ds)
	if set != nil {
		return set.Rep
	}
...
func UnionByRep(rep1, rep2 *gs.Vertex, ds *[]*DisJointSet) *[]*DisJointSet {
	var PI int // index that does merge
	var CI int // index to be merged

	// need to know which element to update
	for k, v := range *ds {
		if gs.SameVertex(rep1, v.Rep) {
			PI = k
		}
		if gs.SameVertex(rep2, v.Rep) {
			CI = k
		}
	}
...
func UnionByVtx(vtx1, vtx2 *gs.Vertex, ds *[]*DisJointSet) *[]*DisJointSet {
...
func (s EdgeSlice) Less(i, j int) bool {
	return s[i].(*gs.Edge).Weight < s[j].(*gs.Edge).Weight
}
...
func SortEdges(graph *gs.Graph) *slice.Sequence {
