...
func Test_JSON_GetEdgeWeight(test *testing.T) {
	...
	for _, testCase := range testCases {
		...
		if weight != testCase.weight {
			test.Errorf("In testgraph1, Expected '%#v'. But %#v", weight, testCase.weight)
		}
	}

	g3 := JSONGraph("../../example_files/testgraph.json", "testgraph.003")
	testCases3 := []struct {
		vertices []string
		weight   float64
	}{
	...
	for _, testCase := range testCases3 {
		...
		if wgt != testCase.weight {
			test.Errorf("In testgraph3, Expected '%#v'. But %#v. %#v, %#v, %#v", testCase.weight, wgt, g3.GetEdgeWeight(g3.FindVertexByID("S"), g3.FindVertexByID("A")), testCase.vertices[0], testCase.vertices[1])
		}
	}
}
