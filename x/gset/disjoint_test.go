...
func Test_makeSet(test *testing.T) {
	...
	s := makeSet(a)
	if len(s) != 1 || !gs.SameVertex(s[a], a) {
...
func Test_MakeGraphSet(test *testing.T) {
	g := gs.FromJSON("../../files/testgraph.json", "testgraph.003")
	s := MakeGraphSet(g)
	// fmt.Printf("%+v", s[0])
	// DisJoint Set (Rep: S) / Vertex Set: [ S ]
	if len(s) != 8 {
		test.Errorf("expected 8 but: %+v", s)
	}
}
...
func Test_SetContains(test *testing.T) {
	...
	if !s[0].SetContains((*vertices)[0].(*gs.Vertex)) {
		test.Errorf("Should return\n%+v", s[0].SetContains((*vertices)[0].(*gs.Vertex)))
	}
}
...
func Test_GetSet(test *testing.T) {
	...
	s := MakeGraphSet(g)
	vertices := g.GetVertices()
	ds := GetSet((*vertices)[0].(*gs.Vertex), s)
...
func Test_FindSet(test *testing.T) {
	...
	s := MakeGraphSet(g)
	vertices := g.GetVertices()
	fs := FindSet((*vertices)[0].(*gs.Vertex), s)
...
func Test_UnionByRep(test *testing.T) {
	...
	s := MakeGraphSet(g)
	vertices := g.GetVertices()
	fs := UnionByRep((*vertices)[0].(*gs.Vertex), (*vertices)[1].(*gs.Vertex), &s)
	// fmt.Println((*vertices)[0].(*gs.Vertex).ID) // S
	// fmt.Println((*vertices)[1].(*gs.Vertex).ID) // A
	// fmt.Printf("%q", *fs)
...
func Test_UnionByVtx(test *testing.T) {
	...
	s := MakeGraphSet(g)
	vertices := g.GetVertices()
	fs := UnionByVtx((*vertices)[0].(*gs.Vertex), (*vertices)[1].(*gs.Vertex), &s)
	// fmt.Println((*vertices)[0].(*gs.Vertex).ID) // S
	// fmt.Println((*vertices)[1].(*gs.Vertex).ID) // A
	// fmt.Printf("%q", *fs)
	// ["DisJoint Set (Rep: S) / Vertex Set: [ S A ]" "DisJoint Set (Rep: B) / Vertex Set: [ B ]" "DisJoint Set (Rep: C) / Vertex Set: [ C ]" "DisJoint Set (Rep: D) / Vertex Set: [ D ]" "DisJoint Set (Rep: T) / Vertex Set: [ T ]" "DisJoint Set (Rep: E) / Vertex Set: [ E ]" "DisJoint Set (Rep: F) / Vertex Set: [ F ]"]
...
func Test_SortEdges(test *testing.T) {
	...
	for _, edge := range *edges {
		fmt.Printf("%v ", edge.(*gs.Edge).Weight)
	}
