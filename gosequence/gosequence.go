// Package gosequence implements sequence operations.
// It uses slice to mitigate the problem of slowness
...
// CopySeqPt returns Copy without dereferencing.
func (s Sequence) CopySeqPt() *Sequence {
	ts := NewSequence()
	for _, v := range s {
		ts.PushBack(v)
	}
