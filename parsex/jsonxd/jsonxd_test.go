...
func TestJSONXD(t *testing.T) {
	...
	*/
	...
	m3, err := js.Get("testgraph.003").Map()
	if err != nil {
		log.Fatal(err)
	}
	...
	*/
	...
	println()
	m3s, err := js.Get("testgraph.003").Get("S").Map()
	if err != nil {
		log.Fatal(err)
	}
	...
	*/
	...
	fmt.Println(reflect.TypeOf(*a)) // gson.JSON
	// fmt.Println(a.Data.(float64))
	// fmt.Println(a.Data.([]float64))
	fmt.Println(a.Float64()) // -1 Can't type assert with float64
	fmt.Println(a.Slice())   // [100] <nil>

	fmt.Println(js.Get("testgraph.003").Get("S").Get("B").Float64Slice())
	// [6 14] <nil>
}

func TestJGD(t *testing.T) {
	m := JGD("../../files/testgraph.json")
	fmt.Println(m)
...
func TestGetGraphMapD(t *testing.T) {
	...
	fmt.Println(ns, reflect.TypeOf(ns))
	/*
	   map[
	   S:map[A:[100] B:[6 14] C:[200]]
	   A:map[S:[15] B:[5] D:[20] T:[44]]
	   B:map[S:[14] A:[5] D:[30] E:[18]]
	   C:map[S:[9] E:[24]]
	   D:map[A:[20] B:[30] E:[2] F:[11] T:[16]]
	   E:map[B:[18] C:[24] D:[2] F:[6] T:[19]]
	   F:map[D:[11] E:[6] T:[6]]
	   T:map[A:[44] D:[16] F:[6] E:[19]]]
